...
public class BidCmd extends HystrixObservableCommand<Bid> {
  ...
  public BidCmd(Setter setter, RestTemplate restTemplate, String providerUrl, int width, int height, String userIp,
  ...
  protected Observable<Bid> construct() {
    ...
    BidRequest bidRequest = BidRequest.builder().width(width).height(height).domain(domain).useragent(userAgent)
        .userip(userIp).build();
